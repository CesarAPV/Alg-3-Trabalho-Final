 ...
 */
void entries_to_heap(Heap *heap,  float *values, int n_points, int flag) {
    No node;
...
/* Funcao que retorna o indice do vizinho a esquerda de um ponto */
int search_neighbor_right(Index *indexer, int i, int n_points) {
...
void update_neighbors(Heap *heap, No *no, float *values, int n_points, int flag) {
    ...
    int r_point = search_neighbor_right(heap->indexer, i, n_points);
    ...
    if (r_point < n_points - 1) {
        ...
        int rr_point = search_neighbor_right(heap->indexer, r_point, n_points);
...
int main(int argc, char *argv[]) {
    ...
    entries_to_heap(heap, values, n_points, flag);
